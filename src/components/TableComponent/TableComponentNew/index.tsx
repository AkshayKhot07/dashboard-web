...
const TableComponentNew = <T extends Record<string, any>>({
  fields,
  columns,
  onRowClick,
  handleSortClick,
    ...
    <div className="w-full overflow-x-auto">
      <table
        className={classNames(
          "table-fixed min-w-full tableComponent",
      ...
      >
        ...
        <thead className="text-text-100 font-bold text-xs  border border-b-text-3">
          <tr className="min-h-[50px] h-[50px] max-h-[50px] rounded-t-xl overflow-hidden">
            {fields.map((item) => {
              return (
                <th
                  key={item.key as string}
                  className={classNames(
                    "p-4 whitespace-nowrap bg-background-300",
                    item.headerClass,
                    {
            ...
            {columns?.length > 0 ? (
              columns.map((item, index) => (
                <tr
                  key={index}
                  className={classNames(
                    "border-b group border-background-6 ",
                    "shadow-top-bottom-light",
                    "rounded-xl"
                  )}
                ...
                >
                  {fields.map((field) => (
                    <td
                      ...
                      className={classNames(
                        field.headerClass,
                        `whitespace-normal text-sm font-normal px-1 transition-all ease-in-out duration-150 group-hover:bg-background-5 bg-background-100`,
                        `first:rounded-tl-xl first:rounded-bl-xl`,
                        `last:rounded-tr-xl last:rounded-br-xl`
                      )}
                    >
